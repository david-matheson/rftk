...
BOOST_AUTO_TEST_CASE(test_Learn)
{
    ...
    BOOST_CHECK( tree.GetPath() == expected_path );
    ...
    BOOST_CHECK( tree.GetIntFeatureParams() == expected_int_params );
    ...
    BOOST_CHECK( tree.GetFloatFeatureParams() == expected_float_params );

    float expected_counts_data[] = { 0, 5, 5, 3, 2, 2, 1, 0 };
    VectorBufferTemplate<float> expected_counts(&expected_counts_data[0], 8);
    BOOST_CHECK( tree.GetCounts() == expected_counts );

    int expected_depths_data[] = { 0, 1, 1, 2, 2, 3, 3, 0 };
    VectorBufferTemplate<int> expected_depths(&expected_depths_data[0], 8);
    BOOST_CHECK( tree.GetDepths() == expected_depths );

    BOOST_CHECK_CLOSE( tree.GetYs().Get(1,0), 1, 0.1 );
    BOOST_CHECK_CLOSE( tree.GetYs().Get(2,1), 0.4, 0.1 );
    BOOST_CHECK_CLOSE( tree.GetYs().Get(2,2), 0.4, 0.1 );
    BOOST_CHECK_CLOSE( tree.GetYs().Get(2,3), 0.2, 0.1 );
    BOOST_CHECK_CLOSE( tree.GetYs().Get(3,2), 0.6666, 0.1 );
    BOOST_CHECK_CLOSE( tree.GetYs().Get(3,3), 0.3333, 0.1 );
    BOOST_CHECK_CLOSE( tree.GetYs().Get(4,1), 1.0, 0.1 );
    BOOST_CHECK_CLOSE( tree.GetYs().Get(5,2), 1.0, 0.1 );
    BOOST_CHECK_CLOSE( tree.GetYs().Get(6,3), 1.0, 0.1 );
}
