...
BOOST_AUTO_TEST_CASE(test_TrySplit)
{
    ...
    BufferCollection bc;
    BOOST_CHECK( maxDepthCriteria.TrySplit(maxDepth-1, 0, bc, 0));
    BOOST_CHECK( !maxDepthCriteria.TrySplit(maxDepth, 0, bc, 0));
    BOOST_CHECK( !maxDepthCriteria.TrySplit(maxDepth+1, 0, bc, 0));
}
...
BOOST_AUTO_TEST_CASE(test_Clone)
{
    ...
    BufferCollection bc;
    BOOST_CHECK( clone->TrySplit(maxDepth-1, 0, bc, 0));
    BOOST_CHECK( !clone->TrySplit(maxDepth, 0, bc, 0));
    BOOST_CHECK( !clone->TrySplit(maxDepth+1, 0, bc, 0));
