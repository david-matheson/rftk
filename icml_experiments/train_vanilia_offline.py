...
def run_experiment(config):
    ...
    out_measurements_filename = ("experiment_data/%s-new-offline-measurements-%s.pkl") % (
    ...
    for number_of_samples, number_of_passes in itertools.product(data_config.data_sizes, data_config.number_of_passes_through_data):
        ...
        for run_id in range(data_config.number_of_runs):
            ...
            predictor = learner.fit(x=X_train, classes=Y_train, 
                                    number_of_features=sklearn_config.number_of_features, 
                                    number_of_trees=sklearn_config.number_of_trees,
                                    max_depth=sklearn_config.max_depth,
                                    min_node_size=sklearn_config.min_samples_split,
                                    # min_child_size=sklearn_config.min_samples_leaf,
                                    number_of_jobs=sklearn_config.number_of_jobs)
            #todo add max_depth, min_samples_split, min_samples_leaf, 
            print "Starting testing", str(datetime.now())

            y_probs = predictor.predict(x=X_test)
            ...
            #         print Y_test[i] -1
            #         print y_hat[i] -1
            #         failed_img = X_test[i,:].reshape(16,16)
